...
**For paper-analyst:** For each sub-protocol described in the paper, what
values does the prover send in each round? What does the verifier send?
Is the ordering explicit? Are commitment steps stated?

**For code-inspector:** For each `prove()` function, trace the data flow:
which values are generated by the prover, which are random challenges, and
is there a commitment or transcript-hash step between them? Flag any prover
value that is used alongside a challenge without being committed first.

**For zkp-auditor:** Compare the paper's protocol rounds against the code's
implementation. For each prover message in the paper's protocol, verify
that the code commits it before using the corresponding challenge.
